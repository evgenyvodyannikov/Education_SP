...
Contoso.SuggestionsApp = function () {
    ...
    //Functions for get the list of suggestions
    var getSuggestions = function () {
        ...
        context.executeQueryAsync(onGetSuggestionsSuccess, onGetSuggestionsFail);
    };

    var onGetSuggestionsSuccess = function (sender, args) {
        var htmlToRender = [];
        var suggestionEnumerator = allSuggestions.getEnumerator();

        while (suggestionEnumerator.moveNext()) {

            var currentSuggestion = suggestionEnumerator.get_current();

            htmlToRender.push('<a onclick="Contoso.SuggestionsApp.display_suggestion(');
            htmlToRender.push(currentSuggestion.get_item('ID'));
            htmlToRender.push(')">');
            htmlToRender.push(currentSuggestion.get_item('Subject'));
            htmlToRender.push('</a><br />');

        }
        $('#suggestions-list').html(htmlToRender.join(''));
    };

    var onGetSuggestionsFail = function (sender, args) {
        alert('Failed to get user name. Error:' + args.get_message());
    };

    //Functions for displaying a suggestion's details
    var displaySuggestion = function (suggestionId) {
        currentSuggestion = suggestionsList.getItemById(suggestionId);
        context.load(currentSuggestion);
        context.executeQueryAsync(onDisplaySuggestionSuccess, onDisplaySuggestionFail);
    };

    var onDisplaySuggestionSuccess = function (sender, args) {
    ...
    var onDisplaySuggestionFail = function (sender, args) {
        alert('Failed to get suggestion. Error:' + args.get_message());
    };

    //Functions for creating a new suggestion
    var createSuggestion = function () {
        var itemCreateInfo = SP.ListItemCreationInformation();
        currentSuggestion = suggestionsList.addItem(itemCreateInfo);
        currentSuggestion.set_item('Subject', $('#subject-input').val().toString());
        currentSuggestion.set_item('Feedback', $('#feedback-input').val().toString());
        currentSuggestion.update();
        context.load(currentSuggestion);
        context.executeQueryAsync(onCreateSuggestionSuccess, onCreateSuggestionFail);
    };

    var onCreateSuggestionSuccess = function () {
        //Nothing to do here.
    };

    var onCreateSuggestionFail = function (sender, args) {
        alert('Failed to create suggestion. Error:' + args.get_message());
    };
...
$(document).ready(function () {
    ...
    Contoso.SuggestionsApp.get_username();
});
...
// SIG // AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFJ3F16mCdQBU
// SIG // rJNrzMTB/WiHqRMzoIIVgjCCBMMwggOroAMCAQICEzMA
...
// SIG // BgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRfTnL7I0ci
// SIG // OSnSq7Cz32hv5yFjzDBeBgorBgEEAYI3AgEMMVAwTqAm
// SIG // gCQATQBpAGMAcgBvAHMAbwBmAHQAIABMAGUAYQByAG4A
// SIG // aQBuAGehJIAiaHR0cDovL3d3dy5taWNyb3NvZnQuY29t
// SIG // L2xlYXJuaW5nIDANBgkqhkiG9w0BAQEFAASCAQDcMAc8
// SIG // uB7XQrJGPyvOz8+SgY6EWAzQ+Bc1ubuyf+ELa7FEv+dJ
// SIG // PGwqDzY5FfRr/wmZ65+si0Ou96gPCE6l8C6a4eXWcc4a
// SIG // cLVY5d025wq7FiiMzn+K6fVifmNbgt8LaqJndo7CW0wF
// SIG // PhnwzT+xseZdj9LdDX5ochAkInTb2Y21+kxYY92/RMf1
// SIG // 6d/QQF3lRgRLEeMtEo/VawCL7GAhrgebVG9oLwu4FjG+
// SIG // rnD8ef2aswZVtFML6BNv2TUoWjZao8P8s3DxFTJ+Qwqh
// SIG // zmZczwzxiWZ+pxS9WalIHSwEioEgVY5jfxcQKRqHzoaA
// SIG // JSvSBDQ2CdK/PX09CNSif8NrAzKboYICKDCCAiQGCSqG
...
// SIG // DQEHATAcBgkqhkiG9w0BCQUxDxcNMTMwNzAxMTgwMDIw
// SIG // WjAjBgkqhkiG9w0BCQQxFgQU1unc1S6H3tpBDrB/L08z
// SIG // 9As2CuowDQYJKoZIhvcNAQEFBQAEggEAhlqOCA9g+vlw
// SIG // 5M2jbKpGv9uWhcZhuTJw1WAHy2GXI/KeGRmoce8XJsDA
// SIG // ToNTQfHf2lq6SJkTeWiJa1H+jS9KwY26tv4BfQPC8D6S
// SIG // ewJWVnetRDUFWzvC39x6oV/8CThsznyjpzy+/DuMv4iP
// SIG // iuRmhu3nhYg57azrsEQrqFWtyiR+glx1F3wLDJYFKa/Q
// SIG // XLpnxSZ7NFwgwJEAtNH3EphV6gsVt2PMVcguMezQEnYh
// SIG // YBZMovz0K0KDo3y2Df/hkrvBehfVmWFomgdpGQ4nVF5q
// SIG // Kab/p8T7bx9JJ5r32VUDbAVBtvE7W/C38IQDMXYKHaue
// SIG // EZm6ENmrMwUjiBHvrhl4HQ==
